...
namespace CISR.Model.Cr
{
	public partial class CrModel
	{
        ...
        public IList<VCal_Record> getVCal_By_TimeId_Keyword_FrameHeadUuid(string pTimeId,string pKeyword,string pFullFrameHeadUuid,string pStatus,OrderLimit orderlimit)
        {
            try
            {
                dbc = IST.Config.DataBase.Factory.getInfo();
                ...
                if (pStatus.Trim().Length > 0)
                {
                    if (pStatus.ToUpper() == "COMPLETE") {
                        sc.Equal(vcal.STATUS, "Y");
                    }
                    else if (pStatus.ToUpper() == "ERROR") {
                        sc.L().Equal(vcal.STATUS, "D").Or().Equal(vcal.STATUS,"E").R();
                    }
                    else if (pStatus.ToUpper() == "WAIT")
                    {
                        sc.Equal(vcal.STATUS, "W");
                    }
        ...
        public int getVCal_By_TimeId_Keyword_FrameHeadUuid_Count(string pTimeId, string pKeyword, string pFullFrameHeadUuid,string pStatus)
        {
            try
            {
                dbc = IST.Config.DataBase.Factory.getInfo();
