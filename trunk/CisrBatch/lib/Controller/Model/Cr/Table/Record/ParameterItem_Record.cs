...
namespace CISR.Model.Cr.Table.Record
{
	...
	[ISTDataBase("CISR")]
	[Serializable]
	public class ParameterItem_Record : RecordBase{
		public ParameterItem_Record(){}
		/*欄位資訊 Start*/
		string _UUID=null;
		...
		public ParameterItem_Record Clone(){
			try{
				return this.Clone<ParameterItem_Record>(this);
			}
			catch (Exception ex){
		...
		public ParameterItem gotoTable(){
			try{
				var dbc = IST.Config.DataBase.Factory.getInfo();
				ParameterItem ret = new ParameterItem(dbc,this);
				return ret;
			}
			catch (Exception ex){
				log.Error(ex);IST.MyException.MyException.Error(this, ex);
		...
		public List<ParameterMonth_Record> Link_ParameterMonth_By_ParameterItemUuid()
		{
			try{
				...
				ret=(List<ParameterMonth_Record>)
										...
										.Equal(___table.PARAMETER_ITEM_UUID,this.UUID))
		...
		public List<Region_Record> Link_Region_By_Uuid()
		{
			try{
				...
				ret=(List<Region_Record>)
										___table.Where(new SQLCondition(___table)
										.Equal(___table.UUID,this.REGION_UUID))
