...
namespace CISR
{
    ...
    {
        public Util.Session.Store ss = new CISR.Util.Session.Store();
        ...
        /// </summary>
        /// <returns></returns>
        public CISR.Controller.Model.Basic.Table.Record.AttendantV_Record getUser()
        {
            if (ss.ExistKey("USER"))
            {
                return (Controller.Model.Basic.Table.Record.AttendantV_Record)ss.getObject("USER");
            }
            return null;
        }
        ...
        public CheckUserStatus checkUser(System.Web.HttpRequest httpRequest)
        {
            if (this.getUser() == null)
            {
                if (httpRequest.Headers["CLOUD_ID"] != null)
                {
                    ...
                    CISR.Controller.Model.Basic.BasicModel bmod = new BasicModel();
                    IList<CISR.Controller.Model.Basic.Table.Record.ActiveConnection_Record> drsAc = new List<CISR.Controller.Model.Basic.Table.Record.ActiveConnection_Record>();
                    ...
                    {
                        var authMaster = "http://" + IST.Config.Cloud.CloudConfigs.GetConfig().AuthMaster + "//router.ashx";
                        ...
                        {
                            CISR.Controller.Model.Basic.Table.Record.ActiveConnection_Record newData = new CISR.Controller.Model.Basic.Table.Record.ActiveConnection_Record();
        ...
        public bool checkProxy(StackFrame sf)
        {
            ...
            if (IST.Config.DirectAuth.DirectAuthConfigs.GetConfig().ProxyPermission == false)
            {
                return true;
            }
            ...
            var json = "{" + IST.Config.DirectAuth.DirectAuthConfigs.GetConfig().NoPermissionAction + "}";
