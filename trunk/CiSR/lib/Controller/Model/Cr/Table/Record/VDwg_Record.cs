...
namespace CISR.Model.Cr.Table.Record
{
	...
	[ISTDataBase("CISR")]
	[Serializable]
	public class VDwg_Record : RecordBase{
		public VDwg_Record(){}
		/*欄位資訊 Start*/
		...
		[ColumnName("DWG_GID",false,typeof(string))]
		...
		public VDwg_Record Clone(){
			try{
				return this.Clone<VDwg_Record>(this);
			}
			catch (Exception ex){
		...
		public VDwg gotoTable(){
			try{
				var dbc = IST.Config.DataBase.Factory.getInfo();
				VDwg ret = new VDwg(dbc,this);
				return ret;
			}
			catch (Exception ex){
				log.Error(ex);IST.MyException.MyException.Error(this, ex);
