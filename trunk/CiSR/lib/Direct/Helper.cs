    ...
    public class Helper
    {
        public class Store
        {
            ...
            public static JObject OutputJObject(List<JObject> jobject, double totalCount)
            {
                ...
                {
                    IST.MyException.MyException.ErrorStaticClass(ex);
            ...
            public static JObject OutputJObject(System.Data.DataTable source, int start, int limit)
            {
                try
                {
                    ...
                    var jarray = new JArray();
                    ret["data"] = IST.Util.JsonHelper.DataTable2JObject(source, start, limit);
                    return ret;
                }
                catch (Exception ex)
                {
            ...
            public static JObject OutputJObject(JObject jobject, double totalCount)
            {
                ...
                {
                    IST.MyException.MyException.ErrorNoThrowExceptionForStaticClass(ex);
                    return null;
                }
            }
        ...
        public class Message
        {
            public class Fail
            {
                ...
                public static JObject OutputJObject(Exception ex)
                {
                    ...
                    err.Append("{");
                    err.Append("\"success\":false,");
                    err.Append("\"message\":\"" + ex.Message.Replace("\"", "").Replace("'", "") + "\"").Replace("~","").Replace("\\","/");
                    err.Append("}");
        ...
        public class Json
        {
            public static List<T> json2Record<T>(JObject jobject) where T : IST.DB.RecordBase, new()
            {
                var t = new List<T>();
                var schema = new T();
                var dataItem = new T();
