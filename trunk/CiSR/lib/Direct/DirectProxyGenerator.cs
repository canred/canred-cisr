...
namespace ExtDirect.Direct
{
    public static class DirectProxyGenerator
    {
        public static string generateDirectApi(string pClassName)
        {
            ...
            {
                /*只解析WebTemplate.DLL*/
                if (allAssembly.ManifestModule.Name.ToUpper() != "CISR.DLL".ToUpper())
                {
                    continue;
                }
                foreach (var theType in allAssembly.GetTypes())
                {
                    ...
                    foreach (var customAttrItem in allCustomAttribute)
                    {
                        ...
                        if (typeof(DirectServiceAttribute) == directType)
                        {
                            ...
                            resStr.Append("{\"" + CISR.Parameter.Config.ParemterConfigs.GetConfig().DirectApplicationName + "." + className + "\":[");
                ...
                foreach (var theType in allAssembly.GetTypes())
                {
                    ...
                    foreach (var customAttrItem in allCustomAttribute)
                    {
                        ...
                        if (typeof(DirectServiceAttribute) == directType)
                        {
                            ...
                            //CISR.Parameter.Config.ParemterConfigs.GetConfig().DirectApplicationName + "." + className;
                            MethodInfo[] methodInfo = theType.GetMethods();
                            var methodList = new List<string>();
