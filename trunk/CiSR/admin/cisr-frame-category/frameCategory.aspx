<%@ Page Title="" Language="C#" MasterPageFile="~/mpStand.Master" AutoEventWireup="true" CodeBehind="frameCategory.aspx.cs" Inherits="Web.admin.cisrFrameCategory.frameCategoryQuery"  EnableViewState="false"  %>
...
Ext.onReady(function () {
    ...
    RegionQuery = Ext.widget({
        xtype: 'panel',
        title: '<img src="' + SYSTEM_URL_ROOT + '/css/images/man.png" style="height:20px;vertical-align:middle;margin-right:5px;">產業別維護',
        frame: true,
        ...
        items: [{
            ...
            items: [{
                ...
                itemId: 'btnQuery',
                handler: function () {
                    //storeFrameCategory.getProxy().setExtraParam('company_uuid', "<%= getUser().COMPANY_UUID %>");
                    storeFrameCategory.getProxy().setExtraParam('keyword', Ext.getCmp('txt_search').getValue());
                    storeFrameCategory.loadPage(1);
                }
            }, {
                xtype: 'label',
                text: '',
                ...
                items:[
                            {
                                 icon: '../../css/custImages/edit.png',
                                    ...
                                    myForm.on('closeEvent', function (obj) {
                                        storeFrameCategory.load();
                                    });
                                }
                                myForm.setTitle('<img src="' + SYSTEM_URL_ROOT + '/css/images/man.png" style="height:20px;vertical-align:middle;margin-right:5px;">產業別【維護】');
            ...
            tbar: [
            {
                text: '<img src="' + SYSTEM_URL_ROOT + '/css/custimages/add.gif" style="height:12px;vertical-align:middle;margin-top:-2px;margin-right:5px;">新增產業別',
                handler: function () {
                    

                    if (myForm == undefined) {
                        myForm = Ext.create('FrameCategoryForm', {});
                        myForm.on('closeEvent', function(obj) {
                            storeFrameCategory.load();
                            obj.unmask();
                        });
                    ...
                    myForm.setTitle('<img src="' + SYSTEM_URL_ROOT + '/css/images/man.png" style="height:20px;vertical-align:middle;margin-right:5px;">產業別【新增】');
                    myForm.uuid = undefined;
                    myForm.show(this);
                    this.up('panel').up('panel').mask();
                }
            }
            ]
        }]
