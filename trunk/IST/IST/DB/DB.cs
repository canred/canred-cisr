    ...
    public class DB :IDisposable
    {
        ...
        public DB(object modelObj) {
            try
            {
                ...
                if (attrs.Length == 1)
                {
                    ...
                    IST.Config.DataBase.IDataBaseConfigInfo a = IST.Config.DataBase.Factory.getInfo();
                    IST.DB.DataBaseConnection obj = new DataBaseConnection(a);
        ...
        public DB(IDataBaseConfigInfo databaseConfigInfo, ADataBaseConnection adatabaseconnection, object modelObj)
        {
            try
            {
                ...
                {
                    dbName = ((IST.Attribute.ISTDataBase)(attrs[0])).getDataBase();
                    IST.Config.DataBase.IDataBaseConfigInfo a = databaseConfigInfo;
        ...
        private void logSQL(ref IDbCommand cmd ,bool isSuccess) {
            var logSQL = IST.Config.DataBase.Factory.getInfo().GetTag("logSQL").ToLower();
            try
            {
                if (logSQL == "false")
                {
