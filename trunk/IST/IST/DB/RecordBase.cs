...
namespace IST.DB
{
    ...
    public abstract class RecordBase 
    {
        ...
        public List<IST.Attribute.ColumnName> getPK()
        {
            try
            {
                List<IST.Attribute.ColumnName> list = new List<IST.Attribute.ColumnName>();
                PropertyInfo[] porps = this.GetType().GetProperties();

                foreach (PropertyInfo prop in porps)
                {
                    object[] attrs = prop.GetCustomAttributes(typeof(IST.Attribute.ColumnName), false);
                    if (attrs.Length == 1)
                    {
                        var isPK = ((IST.Attribute.ColumnName)attrs[0]).IsPK;
                        if (isPK)
                        {
                            list.Add(((IST.Attribute.ColumnName)attrs[0]));
                        }
                    }
                }
        ...
        public List<IST.Attribute.ColumnName> getColumnWithOutPK()
        {
            try
            {
                List<IST.Attribute.ColumnName> list = new List<IST.Attribute.ColumnName>();
        ...
        public string getFieldStringValue(string columnName) {
            ...
            try
            {
                ...
                foreach (PropertyInfo prop in porps)
                {
                    ...
                    if (attrs.Length == 1)
                    {
                        if (((IST.Attribute.ColumnName)attrs[0]).Name.ToUpper() == columnName.ToUpper()) {
        ...
        public List<IST.Attribute.ColumnName> getAllColumn()
        {
            try
            {
                List<IST.Attribute.ColumnName> list = new List<IST.Attribute.ColumnName>();
        ...
        private static int CompareRecord(IST.Attribute.ColumnName x, IST.Attribute.ColumnName y)
        {
            if (x == null)
            {
                if (y == null)
        ...
        public string getTableName() {
            try
            {
                var attrs = this.GetType().GetCustomAttributes(typeof(IST.Attribute.ISTTableView), false);
                if (attrs.Length == 1)
                {
                    return ((IST.Attribute.ISTTableView)attrs[0]).getName();
                }
                return null;
            }
            catch (Exception ex)
