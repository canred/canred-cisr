...
namespace CISR
{
    ...
    /// </summary>
    public class myJobExport : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public string genExcel(string pTimeType,string pAttendantUuid) {
            ...
            try {               
                ...
                var data = mod.getMyUploadJob_TimeType_TimeId_AttendnatnUuid_Keyword_FrameHeadUuid(pTimeType, "", pAttendantUuid, "", "", orderLimit);
                            ...
                            var dataAllTime = (from pTime in data
                                               group pTime by new { pTime.FULL_FRAME_NAME_LIST, pTime.TIME_ID }
                                                   ...
                                                   select new
                                                   {
                                                       grp.Key.TIME_ID,
                                                       grp.Key.FULL_FRAME_NAME_LIST
                                                   }).ToList().Where(c => c.FULL_FRAME_NAME_LIST.Equals(item.FULL_FRAME_NAME_LIST)).OrderBy(c => c.TIME_ID).ToList();
                ...
                foreach (DictionaryEntry item in htSetting) {
                    var rowSetting = sheetSetting.CreateRow(totalSheet);//在工作表中添加一行
