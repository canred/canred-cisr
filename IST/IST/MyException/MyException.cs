...
namespace IST.MyException
{

    [ISTDataBase("ERROR")]
    public static class MyException
    {
        private static IST.Config.DataBase.IDataBaseConfigInfo dbc = null;

        private static void initDB()
        {
            if (dbc == null)
            {
                dbc = dbc = IST.Config.DataBase.Factory.getInfo();
            }
        }
        #region ERROR
        ...
        private static void log(string error_code, string application_name, string attendant_uuid, string errorType, System.Exception ex, bool throwException)
        {
            ...
            {
                initDB();
                var modError = new IST.MyException.Table.ErrorLog(dbc);
                var drError = modError.CreateNew();
                drError.UUID = IST.Util.UID.Instance.GetUniqueID();
        ...
        private static IList<IST.MyException.Table.Record.ErrorLog_Record> getError(string applicationName, string attendantUuid, string keyWord)
        {
            initDB();
            var dtError = new IST.MyException.Table.ErrorLog(dbc);

            var condition = new IST.DB.SQLCondition(dtError);
            ...
            .FetchAll<IST.MyException.Table.Record.ErrorLog_Record>();

            return result;
        }

        private static IList<IST.MyException.Table.Record.ErrorLog_Record> getError(string applicationName)
        {
