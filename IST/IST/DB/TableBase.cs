...
namespace IST.DB
{
    ...
    public class TableBase : iTable
    {
        ...
        public void setDataBaseConfigInfo(IDataBaseConfigInfo config){
            try
            {
                var classType = this.GetType().GetCustomAttributes(typeof(ISTDataBase), true);
                var dataBaseName = ((IST.Attribute.ISTDataBase)(classType[0])).getDataBase().ToUpper();

                if (config.GetDBType(dataBaseName).ToUpper() == "ORACLE")
                {
                    _sqlCreater_ = new IST.DB.SQLCreater.SQLCreaterOracle();
                }
                else if (config.GetDBType(dataBaseName).ToUpper() == "MSSQL")
                {
                    _sqlCreater_ = new IST.DB.SQLCreater.SQLCreaterMSSQL();
                }
                else
                {
                    _sqlCreater_ = new IST.DB.SQLCreater.SQLCreaterMySQL();
        ...
        public TableBase(IDataBaseConfigInfo config,string db) {
            ...
            {
                var classType = this.GetType().GetCustomAttributes(typeof(ISTDataBase), true);
                var dbType = ((IST.Attribute.ISTDataBase)(classType[0])).getDataBase().ToUpper();
        ...
        private void setInit(object modelObj) {
            try
            {
                var attrs = modelObj.GetType().GetCustomAttributes(typeof(ISTDataBase), false);
                string dbName = null;
                if (attrs.Length == 1)
                {
                    dbName = ((ISTDataBase)(attrs[0])).getDataBase();
                    ...
                    {
                        var config = IST.Config.DataBase.Factory.getInfo();
            ...
            try
            {
                ...
                if (limit.getOrderColumn().Count > 0 && limit.getOrderMethod().Count()>0) {
                    ...
                    for (i = 0; i < limit.getOrderMethod().Count(); i++) {
                        ...
                        if (IST.DB.SQLCreater.OrderLimit.OrderMethod.ASC == limit.getOrderMethod()[i]) {
                            _sqlCreater_.OrderASC(limit.getOrderColumn()[i]);
                        }
                        else if (IST.DB.SQLCreater.OrderLimit.OrderMethod.DESC == limit.getOrderMethod()[i])
                        {
        ...
        public void Update_Empty2Null(RecordBase oneData, IST.DB.DB db)
        {
            try
            {
                /*找出我的PK*/
