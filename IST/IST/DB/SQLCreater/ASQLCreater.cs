...
using System.Data;
namespace IST.DB.SQLCreater
{
    /// <summary>
    /// 產生SQL語句的抽像類別
    /// </summary>
    public abstract class ASQLCreater:IDisposable,IST.DB.SQLCreater.ISQLCreater
    {
        public string _SQL_ = "";
        ...
        /// <param name="rc"></param>
        /// <returns></returns>
        public ASQLCreater Update(IST.DB.RecordBase rc) {
            try
            {
                ...
                this._SQL_ += " SET ";
                Int16 no = 1;
                foreach (IST.Attribute.ColumnName col in rc.getColumnWithOutPK())
                {
                    this._SQL_ += col.Name + " = :######,";
                    try
                    {
        ...
        public ASQLCreater Update_Empty2Null(IST.DB.RecordBase rc)
        {
            try
            {
                isComplete = SQLComplete.UNComplete;
        ...
        public ASQLCreater PartialUpdate(IST.DB.RecordBase rc)
        {
            try
            {
                isComplete = SQLComplete.UNComplete;
        ...
        /// <param name="rc"></param>
        /// <returns></returns>
        public virtual ASQLCreater Delete(IST.DB.RecordBase rc)
        {
            try
            {
                isComplete = SQLComplete.UNComplete;
        ...
        /// <param name="rc"></param>
        /// <returns></returns>
        public virtual ASQLCreater Insert(IST.DB.RecordBase rc)
        {
            try
            {
                isComplete = SQLComplete.UNComplete;
                ...
                Int16 no = 1;
                this._SQL_ += " ( ";
                foreach (IST.Attribute.ColumnName col in rc.getAllColumn())
                {
        ...
        public virtual ASQLCreater Insert_Empty2Null(IST.DB.RecordBase rc)
        {
            try
            {
                isComplete = SQLComplete.UNComplete;
            ...
            try
            {
                ...
                foreach (string tmp in arrSql)
                {
                    ...
                    if (no <= _SelfPARAMETER_.Count)
                    {
                        ...
                        if (this.ToString() == "IST.DB.SQLCreater.SQLCreaterMySQL")
                        {
