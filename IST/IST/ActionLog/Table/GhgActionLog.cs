...
using IST.ActionLog.Table.Record;
namespace IST.ActionLog.Table
{
    [ISTDataBase("ActionLog")]
    [ISTTableView("ACTION_LOG", true)]
    ...
    private IList<ActionLog_Record> _All_Record = new List<ActionLog_Record>();
		...
		/*利用物件自已的AllRecord的資料來更新資料行*/
        public void UpdateAllRecord(IST.DB.DB db)
        {
			try{
		...
		/*利用物件自已的AllRecord的資料來新增資料行*/
        public void InsertAllRecord(IST.DB.DB db)
        {
			try{
		...
		/*利用物件自已的AllRecord的資料來刪除資料行*/
        public void DeleteAllRecord(IST.DB.DB db)
        {
			try{
