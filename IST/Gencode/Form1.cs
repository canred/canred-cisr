...
namespace Gencode
{
    public partial class Form1 : Form
    {
        ...
        private void genCodeModel(IList<XDBSource> xdb) {
            ...
            sw.WriteLine("using IST.Attribute;");
            sw.WriteLine("using IST.DB;  ");
            sw.WriteLine("using log4net;  ");
            sw.WriteLine("using System.Reflection;  ");
            sw.WriteLine("using IST.DB.SQLCreater;  ");
            ...
            sw.WriteLine("\t[ISTDataBase(\"" + objectName + "\")]");
            ...
            sw.WriteLine("\t\tprivate IST.Config.DataBase.IDataBaseConfigInfo dbc = null;");
            ...
            foreach (var table in xdb)
            {
                ...
                if (pkString != "")
                {
                    ...
                    sw.WriteLine("\t\t\ttry{");
                    sw.WriteLine("\t\t\t\tdbc = IST.Config.DataBase.Factory.getInfo();");
                    ...
                    sw.WriteLine("\t\t\t\tlog.Error(ex);IST.MyException.MyException.Error(this, ex);"); 
                    ...
                    string objTableName)
        {
            ...
            sw.WriteLine("{");
            sw.WriteLine("\t[ISTRecord]");
            sw.WriteLine("\t[ISTTableView(\"" + _tableName + "\", " + isTable + ")]");
            sw.WriteLine("\t[ISTDataBase(\"" + source.objectName.ToUpper().Split('.')[0].Replace("\"", "") + "\")]");
            ...
            sw.WriteLine("\t\t\ttry{");
            sw.WriteLine("\t\t\t\tvar dbc = IST.Config.DataBase.Factory.getInfo();");
                    ...
                    string objTableName)
        {
            ...
            sw.WriteLine("using IST.Attribute;  ");
            sw.WriteLine("using IST.DB;  ");
            sw.WriteLine("using IST.Config.DataBase;  ");
            sw.WriteLine("using IST.DB.SQLCreater;  ");
            ...
            sw.WriteLine("\t/*固定物件*/");
            sw.WriteLine("\t//IST.DB.SQLCreater.ASQLCreater sqlCreater = null;");
            ...
            if (source.ConstraintCol.Count > 0)
            {
                ...
                sw.WriteLine("\t\t\t}");
                sw.WriteLine("\t\t\tcatch (Exception ex){");
                sw.WriteLine("\t\t\t\tlog.Error(ex);IST.MyException.MyException.ErrorNoThrowException(this, ex);");
                //sw.WriteLine("\t\t\t\tthrow ex;");
            ...
            string objTableName)
        {
            ...
            sw.WriteLine("namespace " + txtNamespace.Text + ".Model." + modelName + ".Table");
            sw.WriteLine("{");
            sw.WriteLine("\t[ISTDataBase(\"" + source.objectName.ToUpper().Split('.')[0] + "\")]");
            sw.WriteLine("\t[ISTTableView(\"" + _tableName + "\", " + isTable + ")]");
