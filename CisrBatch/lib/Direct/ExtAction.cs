...
namespace ExtDirect.Direct
{
    public class ExtAction : IAction
    {
        ...
        public string ExecuteNormalAction2(Request request)
        {
            ...
            /*ActionLog*/
            IST.ActionLog.ActionLog.SetActionLog(param, theMethod);
            /*反射呼叫指定的方法*/
            try
            {
                //字串的處理方式
        ...
        public JObject ExecuteNormalAction2JObject(Request request)
        {
            ...
            try
            {
                ...
                {
                    IST.MyException.MyException.ErrorNoThrowExceptionForStaticClass(new Exception(request.action + "." + request.method + "發現呼叫action但無法執行，可能是參數錯誤!"));
                }
                if (action == DirectAction.TreeStore)
                {
